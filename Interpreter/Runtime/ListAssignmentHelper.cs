...
{
    // Shared body of ListAssignmentNodeVisitor — `arr[i] = v` (plus
    // compound forms). The visitor evaluates target/index/value
    // sub-expressions then calls Apply(); the VM's OP_SET_INDEX
    // pre-evaluates them into slots.
    public static class ListAssignmentHelper
    {
        public static RuntimeResult Apply(
            ListAssignmentNode node, Context context,
            RuntimeValue targetList, RuntimeValue indexValue, RuntimeValue valueToAssign)
        {
                ...
                switch (node.AssignmentToken.Type)
                {
                    ...
                    case TokenType.NULL_COALESCE_EQ:
                        if (currentValue.Type == RuntimeValueType.Null)
                            (result, error) = (valueToAssign.SetContext(context).SetPos(node.PositionStart, node.PositionEnd), null);
                        else
            ...
            var (_, setErr) = targetList.ListSet(indexValue, finalValue);
            if (setErr != null) return res.Failure(setErr);
