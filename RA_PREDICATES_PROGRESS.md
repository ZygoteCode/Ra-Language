...
### P4 — type-narrowing guards (THE CROWN JEWEL)

Scaffolding is already in place: `FunctionDefinitionNode.NarrowsParamName/NarrowsToType/NarrowsNegated` and the matching `PredicateValue` fields exist; `FunctionDefinitionHelper.Apply` already threads node→leaf. **Two pieces remain: (a) detection, (b) analyzer integration.**

**(a) Detection — populate the node fields.** A narrowing predicate's body is exactly `param is T` / `param is not T` where `param` is the (single) predicate parameter. Detect in `FunctionDefinitionHelper.Apply` (or a small pass) BEFORE building the leaf:
- Unwrap the body: for an arrow predicate the body is the expression directly; for a block body, only treat a single `ret <expr>` (or trivial `{ <expr> }`) as a guard.
- If the (unwrapped) expression is an `IsTypeNode` (`Parser/Nodes/Operations/IsTypeNode.cs`, fields `Expression`, `TestedType`, `Negated`) whose `Expression` is a `VariableAccessNode` naming the predicate's parameter → set `node.NarrowsParamName = param`, `node.NarrowsToType = isNode.TestedType`, `node.NarrowsNegated = isNode.Negated`.
- `PredicateValue.Leaf(... narrowsParam, narrowsTo, narrowsNegated)` already accepts these — pass them.

**(b) Analyzer integration — flow-type the call.** Ra already narrows on inline `is`:
- `Types/TypeSystem.cs:735` `NarrowToType(ctx, declared, tested)` and `:774` `NarrowAwayFromType(...)`.
- `Interpreter/Runtime/Narrowing/NarrowingAnalyzer.cs` walks scopes, refines types after `is`, reports impossible tests, checks union exhaustiveness.
- `IsTypeNodeVisitor` evaluates `x is T` at runtime via `TypeSystem.IsRuntimeTypeMatch`.

Extend `NarrowingAnalyzer` so a condition that is a **call** `p(v)` — where `p` resolves to a named predicate whose `NarrowsParamName != null` and `v` is a `VariableAccessNode` — narrows `v` to `NarrowsToType` in the then-branch (and `NarrowAwayFromType` in the else), mirroring exactly how it treats an inline `v is T`. Resolve `p` through the same symbol table the analyzer already uses; read the narrowing metadata off the bound `PredicateValue` (or the predicate's declaration node).

**Conservative v1 boundary (document it):** narrowing flows only for a **direct call to a single narrowing predicate** used as the whole condition (`if p(v)`, `if !p(v)`). Composites (`if (p & q)(v)`) and predicates passed as values do NOT narrow (they still work as runtime bools). This keeps it tractable and is the documented limit. The runtime semantics of a narrowing predicate are just `x is T` returning bool — already correct; P4 is purely the compile-time flow-typing.

**Tests:** then-branch sees the narrowed type (member access / assignment that's only valid on `T` type-checks); `is not`/`!p` narrows the else-branch; an impossible guard warns; a non-guard predicate does not narrow.

### P5 — tests, bench, docs, memory

- `tests_predicates.ra` — hard-assert suite (`check`/`check_eq` that `print "OK ..."` on success and `throw` on failure; import header `import std.prelude.*;`). Cover: direct call, anonymous literal, `& | !` + short-circuit (use a side-effect counter to prove short-circuit), `!!` fold, block body, all methods, `pred<T>` assignment + `fn(T)->bool` interop, every stdlib HOF, combinators, narrowing then/else/impossible, and negative cases (non-bool body rejected, compose-with-non-callable rejected, unknown method rejected). **Comments must be on their own line** (see gotcha #2).
...
| Type guards / narrowing | ⏳ **P4** (scaffolding in place) |
