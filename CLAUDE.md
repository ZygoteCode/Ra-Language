...
Dotted imports under `std` resolve to a **physical** `.ra` file (`import std.io` → `std/io.ra`), a **virtual module** of categorised built-ins (`import std.prelude.io`), a **package** aggregate (`import std.prelude`, `import std.prelude.*`, `import std.sys`, bare `import std`, or any physical std directory), or a clear not-found error. A trailing `.*` (`ModuleSpecifier.IsWildcard`, parsed in `Parser.Imports.cs`) forces the package interpretation. The std taxonomy — every built-in function mapped to a `std.prelude.*` / `std.sys.*` module — lives in [Interpreter/Modules/StdLibrary.cs](Interpreter/Modules/StdLibrary.cs), **derived** from `BuiltInRegistry` group tags (captured by the `RegisterGrouped(...)` wrappers in `BuiltInRegistry.EnsureInitialized`) plus async/stream `Names` and a small override table — never a hand-kept list. Resolution precedence and synthesis live in `ModuleManager.TryPlanStdImport` / `LoadVirtualModule` / `LoadPackage`. **There is no auto-prelude:** built-in *functions* are import-only (`import std.prelude.*`, etc.) and must be imported by every file/module that uses them. `Program.InitializeSymbolTable` builds two stores — `BuiltinSymbolTable` (full set: functions + annotation types + ADTs; the synthesis + tooling source, NOT a runtime parent) and `CoreSymbolTable` (the only always-available scope: annotation types + `Result`/`Option`, the runtime parent of `GlobalSymbolTable` and every module). `ModuleManager` takes two providers: `coreProvider` (module parent) and `functionStoreProvider` (synthesis). A local `var`/`let` may shadow an imported built-in function — `DeclarationHelper.IsRealRedeclaration` only rejects redeclaring a *non*-`BuiltInFunctionValue` (so `var e` / `let sign` keep working after `import std.prelude.*`). The `.rac` packager skips virtual std imports (`StdLibrary.IsVirtualStdPath`); `--compile --std-root <dir>` targets a specific std tree. `--selftest-stdlib` audits that the taxonomy covers the live built-in set exactly (no orphan built-ins, no phantom entries). **All test/bench/fixture `.ra` files carry explicit imports** (`import std.prelude.*;`, plus `import std.sys.*;` for FFI/asm). Full design in [RA_STDLIB_PRELUDE_DESIGN.md](RA_STDLIB_PRELUDE_DESIGN.md); tests at `tests/stdlib/`.
...
First-class **predicates** — `pred even(n) => n % 2 == 0`, `pred adult(p) { ret p.age >= 18 }`, anonymous `pred(n) => n > 100` — are composable boolean functions, **reusing the existing function + call + operator dispatch with zero new opcodes, zero new AST node kinds, zero new visitors**. A `pred` lowers to an ordinary `FunctionDefinitionNode` (`IsPredicate = true`, `bool` return contract enforced — RA0209 on a non-bool return); `FunctionDefinitionHelper.Apply` wraps the produced `FunctionValue` in a leaf [`PredicateValue : BaseFunctionValue`](Interpreter/Values/Functions/Predicates/PredicateValue.cs) (`RuntimeValueType.Predicate`) whose call result is always a `BooleanValue`. **Composition** is the operators `&` / `|` / `!` (riding the generic `BAnd`/`BOr`/`NotB` VM paths via `BitwiseAndedBy`/`BitwiseOredBy`/`Notted`; RHS auto-lifts a plain `fn->bool`; short-circuit composites; compose-time folds `!!p→p` + constant folds against `always_true`/`always_false`) plus the methods `.negate/.xor/.implies/.iff/.test` ([`BoundPredicateMethodValue`](Interpreter/Values/Functions/Predicates/BoundPredicateMethodValue.cs) via `MemberAccessHelper`). `and`/`or`/`not` stay boolean control-flow (NOT overloaded — they'd diverge AST vs IR); `^` stays exponent (XOR is `.xor`). `pred<T>` is type sugar for `fn(T)->bool` (`Parser.Types.cs`). **Stdlib HOFs** (`filter/reject/find/find_index/any/all/none/count/partition/take_while/drop_while` in `CollectionBuiltins`; `pred_all/pred_any/pred_none/negate/always_true/always_false` in `PredicateBuiltins`, group `func`) accept any callable. **Type-guard predicates**: a `param is T` body is detected by the parser (`DetectNarrowingGuard`) and the `NarrowingAnalyzer` flags impossible/redundant guard calls `p(v)` like an inline `v is T` (diagnostics-only — Ra has no runtime flow-typing). RA0416 on composing with a non-callable. **Naming gotcha:** the combinators use the `pred_` prefix because bare `all_of`/`any_of`/`none_of` are reserved built-in validator annotation names. Design in [RA_PREDICATES_DESIGN.md](RA_PREDICATES_DESIGN.md). Hard-asserted tests at `tests/functions/test_predicates.ra`; microbench at `bench/bench_predicates.ra`.

### Callables (HOFs + signature diagnostics)

Ra's callable model is unified on `BaseFunctionValue` (lambdas `|x|` / `fn` / `pred`, bound methods, extension methods, multicast/partial/composed delegates) through one chokepoint (`FunctionCallExecutor.Invoke`), with a variance-aware structural `fn(P…)->R` type system. Two increments documented in [RA_CALLABLES_DESIGN.md](RA_CALLABLES_DESIGN.md): **(1) transform HOFs** over lists in `CollectionBuiltins.cs` — `map/flat_map/for_each/reduce/fold/sort_with/sort_by/group_by/zip_with/min_by/max_by/sum_by`, each taking any callable (free functions, mirroring the predicate HOFs; group `collections`). **(2) compiler-grade callable signature diagnostics** (RA0404): `TypeSystem.TryDescribeFunctionMismatch` reports `expected 'fn(int)->bool' … found 'fn(any,any)' …` + a hint, fired via the shared [`CallableDiagnostics`](Interpreter/Runtime/CallableDiagnostics.cs) helper at every typed-callable binding site — declaration (`DeclarationHelper`), assignment (`AssignmentHelper`), plain-fn args (`FunctionValue`), class-method args (`BoundClassMethodValue`/`Group`), extension/trait args (`MethodCallBinder`/`BoundExtensionMethodGroupValue`); the terminal "no matching overload" sites re-examine candidates via `MethodCallBinder.DescribeCallableArgMismatch`. **Target-typing** is acceptance+diagnostic-layer only (an untyped `|x|` is accepted into a `fn(int)->bool` slot; only statically-knowable arity/explicit-type conflicts are flagged) — no runtime param-stamping (lambda aliasing fragility). **(3) Fluent method form** (always-on, Dart/C#/JS-style): every list HOF/op is ALSO a method — `xs.map(f).filter(p).sort_by(k).take(3)` — via [`CollectionMethods`](Interpreter/Runtime/CollectionMethods.cs) (method→builtin map; LINQ/Dart aliases `where`/`some`/`every`/`each`/`distinct`/`flatMap`/…) binding a [`BoundCollectionMethodValue`](Interpreter/Values/Functions/BoundCollectionMethodValue.cs) that prepends the receiver; resolved in `MemberAccessHelper` AFTER user `extend` methods (so extensions win). Enabler: the member-access parser ([Parser.Expressions.cs](Parser/Parser.Expressions.cs)) now accepts a **keyword after `.`** as a contextual member name (was a hard parse error). Lists only. Tests: `tests/functions/test_callable_diagnostics.ra`, `tests/collections/test_hof_transform.ra`, `tests/collections/test_fluent_methods.ra`.
